...
def plot_k_means(X, K, index_word_map, max_iter=20, beta=1.0, show_plots=True):
    ...
    # print out the words in each clusters
    hard_responsibilities = np.argmax(R,
                                      axis=1)  # is an N-size array of cluster identities = argmax of the responsibilities matrix across each row/
    # let's "reverse" the order so it's cluster identity -> word index
    cluster2word = {}  # a dictionary that points from cluster identity to all the words that belong to that cluster
    ...
    # once we've collected all the words, we can loop through the dictionary and print out each list of words 1 by 1.
